              ...
              <div className="patterns-by-section">
                {groupedPatternsInSections.map((section, sectionIndex) => (
                  <div key={`section-${sectionIndex}`} className="pattern-section mb-6">
                    <h2 id={`section-${sectionIndex}`} className="section-title">{section.sectionTitle}</h2>
                    
                    <div className="patterns-grid">
                      {section.groupedPatterns.map((patternGroup) => {
                        // Get the group characteristics
                        const groupHasMultiplePlatforms = patternGroup.length > 1;
                        
                        return patternGroup.map((pattern, patternIndex) => {
                          ...
                          const isSubsequentPlatformVariant = patternIndex > 0;
                          ...
                          return (
                            <div key={pattern.id} className="card pattern-card">
                              ...
                              {isSubsequentPlatformVariant && (
                                <h4 className="pattern-subtitle">For {pattern.category} platforms</h4>
                              )}
                              
                              {/* First variant with multiple platforms should indicate it's for that specific platform */}
                              {!isSubsequentPlatformVariant && groupHasMultiplePlatforms && (
                                <h4 className="pattern-subtitle">For {pattern.category} platforms</h4>
                              )}
